...
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkText)
{
    ...
    var classMarkup = tableClassName ? ' class="' + tableClassName + '"' :
                                       '';
    var styleMarkup = 'style="font-size:11px;"';
    var tbl = '<table' + idMarkup + classMarkup + styleMarkup + '>{0}{1}</table>';
        ...
        else
        {
            if(headers)
            {
                ...
                for (var i = 0; i < parsedJson.length; i++)
                {
                    ...
                    {
                        var replacedata = parsedJson[i][headers[j]];                        
                        var value = "";
                        if (replacedata != null)
                        {
                            var len = replacedata.toString().length;
                            if (len == 5) {                               
                                value = replacedata.toString().replace(/\./g, ':');
                            }
                            else if (len == 4)
                            {
                                var temp = replacedata.toString().split(".");
                                var leng = temp[0].toString().length;
                                if (leng == 2)
                                {
                                    value = replacedata.toString().replace(/\./g, ':')+'0';
                                }
                                else {
                                    value = replacedata.toString().replace(/\./g, ':');
                                }                               
                            ...
                            else if (len == 3)
                            {
                                ...
                                value = replacedata.toString().replace(/\./g, ':') + ':00';
                            }
                        }
                        else {
                            value = replacedata;
